...
public class PaymentController {
    ...
    public String updateOrderStatus(HttpServletRequest request) throws UnsupportedEncodingException {
        log.info("支付宝异步回调....");
        //获取到订单的支付状态，修改订单状态
        //request对象中会自带out_trade_no、trade_no、trade_status
        //拿到阿里异步回调我们这个方法的所有参数
        ...
        //1、验证合法性，验签
        try {
//            boolean signVerified = AlipaySignature //验签不稳点，所以注释，开发中要签签
...
//            }
    ...
    * 处理支付请求
    ...
    public String paymentPage(AlipayRequestVo vo,Integer id) throws AlipayApiException {
        ...
        OrderInfo orderInfo = orderService.getOrderById(id);//去数据库拿到订单信息
        //提交上来的金额与数据库中查出来的金额是否一样,金额(浮点型)用来计算必须是BigDecimal类型
        BigDecimal subtract = orderInfo.getTotalAmount().subtract(new BigDecimal(vo.getTotalAmount()));
        if(subtract.intValue() == 0){
            //价格一样
            //给阿里去发支付请求
            //1、获得初始化的AlipayClient
            //以下使用的都是沙箱环境,若是要是真实环境,改变所传的参数即可
            AlipayClient alipayClient =
                    new DefaultAlipayClient(
                            GmallAlipayConfig.gatewayUrl,//支付宝个人沙箱路径
                            GmallAlipayConfig.app_id,//沙箱的id
                            GmallAlipayConfig.merchant_private_key,//RSA(非对称加密算法)私钥
                            "json",//数据传输格式
                            GmallAlipayConfig.charset,//编码
                            GmallAlipayConfig.alipay_public_key,//RSA(非对称加密算法)公钥
                            GmallAlipayConfig.sign_type);//使用那种加密算法(这里使用的RSA)
            ...
            return result;//该结果集,是一段form表单,自动执行
        }else{
            return "订单非法";
        }
    }
